...
function isActionsOpen() {
  return (
    ($("#actions-hide span").length &&
      $("#actions-hide span").hasClass("rotateRight")) ||
    ($("#actions-hide img").length &&
      $("#actions-hide img").hasClass("rotateRight"))
  );
}
...
function createPlaybackSpeedSlider(defaultPlaybackSpeed) {
  $("#speed-input").slider({
    ...
    min: 1000,
    max: 2000,
    value: defaultPlaybackSpeed * 1000 + 500,
    step: 250,
    change: function (event, ui) {
      gw.setAnimationDuration(2200 - ui.value);
    },
    slide: function (event, ui) {
      speedVal = 0.5 + (ui.value - 1000) / 1000;
      $("#viz-speed-value").html(
        Math.round((speedVal + Number.EPSILON) * 100) / 100 + "x"
      );
    },
  });
}
$(function () {
  ...
  $("#other-modes a").click(function () {
    ...
    var tmp = $("#mode-button")
      .html()
      .substring(0, $("#mode-button").html().length - 2);
  ...
  $(".electure-end").click(end_eLecture);
  ...
  $(document).keydown(function (event) {
    if (event.which == 32) {
      ...
      if (mode != "e-Lecture") stepBackward();
    } else if (event.which == 39) {
      if (mode != "e-Lecture") stepForward();
